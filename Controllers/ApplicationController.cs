...
namespace Xray.Controllers
{
    ...
    public class ApplicationController : Controller
    {
        ...
        {
            DeployedApplicationList deployedApplications = await client.QueryManager.GetDeployedApplicationListAsync(nodeName);
            ApplicationList applications = await client.QueryManager.GetApplicationListAsync();
            ...
            {
                ServiceList services = await client.QueryManager.GetServiceListAsync(deployedApplication.ApplicationName);
                DeployedServiceReplicaList deployedReplicas = await client.QueryManager.GetDeployedReplicaListAsync(nodeName, deployedApplication.ApplicationName);
                
                var groups = deployedReplicas.GroupBy(x => x.ServiceName);
                ...
                foreach (var group in groups)
                {
                    ...
                    foreach (DeployedServiceReplica item in group)
                    {
                        ...
                        {
                            DeployedStatefulServiceReplicaDetail detail = await client.QueryManager.GetDeployedReplicaDetailAsync(nodeName, statefulDeployedReplica.Partitionid, statefulDeployedReplica.ReplicaId) as DeployedStatefulServiceReplicaDetail;
                            ServiceReplicaList replicaList = await client.QueryManager.GetReplicaListAsync(detail.PartitionId, detail.ReplicaId);
                            
                            Replica replica = replicaList.FirstOrDefault();

                            replicaModels.Add(new ReplicaModel(
                    ...
                    serviceModels.Add(new DeployedServiceModel(
                        new ServiceModel(
                            group.Key.ToString(),
                ...
                applicationModels.Add(new DeployedApplicationModel(
